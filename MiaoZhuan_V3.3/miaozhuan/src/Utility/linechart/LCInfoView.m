...
- (id)initWithFrame:(CGRect)frame {
    ...
    {
        UIFont *fatFont = [UIFont systemFontOfSize:15];
        
        self.infoLabel = [[UILabel alloc] init];
        self.infoLabel.backgroundColor = [UIColor clearColor];
        self.infoLabel.font = fatFont;
//        self.infoLabel.backgroundColor = AppColorRed;
//        self.infoLabel.layer.cornerRadius = 5.f;
...
- (void)layoutSubviews
{
    ...
    self.infoLabel.frame = CGRectMake(self.bounds.origin.x + 3.5, self.bounds.origin.y, self.infoLabel.frame.size.width, self.infoLabel.frame.size.height);
}
...
#pragma clang diagnostic pop
    ...
    s.width += 2 * SHADOWSIZE + 7 - 2;
    s.width = MAX(s.width, HOOK_SIZE * 2 + 2 * SHADOWSIZE + 5);
...
//    CGContextSetShadow(c, CGSizeMake(0.0, SHADOWSIZE), SHADOWBLUR);
    ...
    CGContextAddRoundedRectWithHookSimple(c, theRect, 5);
	CGContextFillPath(c);
...
- (void)recalculateFrame {
...

    {
        ...
        theFrame.origin.x = round(self.tapPoint.x - ((theFrame.size.width - 2 * SHADOWSIZE)) / 2.0) + 2;
    }
    self.frame = theFrame;
}
