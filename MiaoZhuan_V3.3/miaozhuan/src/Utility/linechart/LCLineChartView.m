...
@property LCInfoView *infoView_ex;
...
- (void)setDefaultValues {
    ...
    self.axisLabelColor = AppColor(85);
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat availableWidth = self.bounds.size.width - 2 * PADDING - self.yAxisLabelsWidth - CR_EXTRALINE_ADD;
    ...
    CGContextSetLineWidth(c, CR_EXTRALINE_WIDTH);
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [step drawInRect:CGRectMake(yStart - CR_EXTRALINE_ADD, y - h / 2, self.yAxisLabelsWidth - 6, h) withFont:self.scaleFont lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentRight];
#pragma clagn diagnostic pop

        [AppColor(204) set];
//        CGContextSetLineDash(c, 0, dashedPattern, 2);
        CGContextMoveToPoint(c, xStart - CR_EXTRALINE_ADD, round(y) + 0.5);
        ...
        if (self.drawsDataLines) {
            ...
            if(data.itemCount >= 2) {
                ...
                for(NSUInteger i = 1; i < data.itemCount; ++i) {
                    ...
                    if(xDiff != 0) {
                        CGFloat xSmoothing = self.smoothPlot ? MIN(15,xDiff) : 0;
...
//                CGContextSetStrokeColorWithColor(c, [[UIColor redColor] CGColor]);
                ...
                CGContextSetLineWidth(c, CRLINE_WIDTH);
                CGContextStrokePath(c);
        ...
        if (self.drawsDataPoints) {
          if (data.drawsDataPoints) {
            ...
            for(NSUInteger i = 0; i < data.itemCount; ++i) {
                ...
                CGContextFillEllipseInRect(c, CGRectMake(xVal - CR_POINTS_R_OUTTER/2, yVal - CR_POINTS_R_OUTTER/2, CR_POINTS_R_OUTTER, CR_POINTS_R_OUTTER));
                [data.color setFill];
                CGContextFillEllipseInRect(c, CGRectMake(xVal - CR_POINTS_R_INNER/2, yVal - CR_POINTS_R_INNER/2, CR_POINTS_R_INNER, CR_POINTS_R_INNER));
                {
                    CGFloat brightness;
                    CGFloat r,g,b,a;
                ...
                CGContextFillEllipseInRect(c, CGRectMake(xVal - 1.5, yVal - 1.5, 3, 3));
            } // for
...
- (void)showIndicatorForTouch:(UITouch *)touch
{
    ...
    if(! self.infoView)
    {
        ...
        self.infoView.color = AppColorRed;
    ...
    if (self.data.count > 1)
    {
        if(! self.infoView_ex)
        {
            self.infoView_ex = [[LCInfoView alloc] init];
            self.infoView_ex.alpha = 0;
            self.infoView_ex.color = RGBACOLOR(80, 155, 240, 1);
            [self addSubview:self.infoView_ex];
        }
    }
...
//    closestIdx == self.selectedIdx
    if(closest == nil || closestData == self.selectedData)
    {
        if (dataIndex == 0)
        {
            [self hideIndicator];
    ...
    LCInfoView *holderView = dataIndex == 0?self.infoView:self.infoView_ex;
    
    holderView.infoLabel.text = closest.dataLabel;
    holderView.tapPoint = closestPos;
