...
@property (nonatomic, strong) NSStringBlock stateBlock;
...
- (id)init {
    ...
    if (self) {
        ...
        self.lastState = [standard objectForKey:MMLastState];
    }
    return self;
}
...
- (void) getState:(NSStringBlock)stateBlock
{
    self.stateBlock = [stateBlock copy];
    [self startLocation];
}
...
- (void) getState:(NSStringBlock)stateBlock error:(LocationErrorBlock) errorBlock{
    self.stateBlock = [stateBlock copy];
    self.errorBlock = [errorBlock copy];
    [self startLocation];
}
...
- (void)mapView:(MKMapView *)mapView didUpdateUserLocation:(MKUserLocation *)userLocation
{
    ...
    [standard setObject:@(self.lastCoordinate.longitude) forKey:@"kMZLongitude"];
    [standard setObject:@(self.lastCoordinate.latitude) forKey:@"kMZLatitude"];
    ...
    CLGeocodeCompletionHandler handle = ^(NSArray *placemarks,NSError *error)
    {
        for (CLPlacemark * placeMark in placemarks)
        {
            ...
            [standard setObject:self.lastState forKey:MMLastState];
