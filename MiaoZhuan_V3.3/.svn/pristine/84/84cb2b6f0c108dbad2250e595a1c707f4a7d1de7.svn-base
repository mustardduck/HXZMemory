...
-(void)layoutSubviews {
    ...
    if ([customText length] != 0) {
        ...
        percentLabel.attributedText = [RRAttributedString setText:percentLabel.text font:Font(25) range:NSMakeRange(percentLabel.text.length - 1, 1)];
    } else {
...
//    
    percentLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 85, 85)];
    [percentLabel setFont:[UIFont systemFontOfSize:45]];
    [percentLabel setTextColor:AppColorWhite];
    ...
    imageLayer.frame = CGRectMake(1, 1, 118, 118);
...
-(void)delayedDraw:(NSNumber *)newPercentage {
    ...
    if (perc != finalPercent && !dragging) {
        [self performSelector:@selector(delayedDraw:) withObject:[NSNumber numberWithInt:perc] afterDelay:.01];
    } else {
        currentAnimating = NO;
    }
}
...
- (void)setPercent:(int)percent animated:(BOOL)animated {
    if (animated) {
        ...
        [self performSelector:@selector(delayedDraw:) withObject:[NSNumber numberWithInt:oldPercent] afterDelay:0.3];
    } else {
